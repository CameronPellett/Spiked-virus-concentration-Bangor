...
pellet <- spike_ami_pel|>
 filter(method %in% c("PEG", "BE-PEG", "Pellet", "Pellet_BE"))|>
  mutate(BE = str_detect(method, "BE"),
         ...
         method = case_when(method == "BE-PEG" ~ "BE-PEG concentrate",
                            method == "PEG" ~ "PEG concentrate",
                            TRUE ~ method))
...
```{r}
qq_pellet/ 
  qq_pellet_unspike  + plot_annotation(tag_levels = "a")
```


Figure S5: qq-plot of log~10~ recovery from spiked (a), and unspiked samples (b), by concentration methods including pellet extraction and quantification.
